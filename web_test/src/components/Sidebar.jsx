...
import { sessionRoleLabel } from "../services/api";
import "./Sidebar.css";

export default function Sidebar({ isOpen, onClose, session, onLogout }) {
  const { conversations, activeId, newChat, selectChat, removeChat, clearAllChats } =
    useChat();
      ...
      <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-footer">
          <div className="sidebar-session-card">
            <div className="sidebar-session-label">Connecte</div>
            <div className="sidebar-session-role">
              {session?.username || "—"} · {sessionRoleLabel(session?.role)}
            </div>
          </div>
          <button className="sidebar-footer-btn" onClick={() => clearAllChats()}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2m3 0v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6h14" />
            </svg>
            Masquer tout (serveur)
          </button>
